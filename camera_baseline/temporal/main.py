...
from camera_baseline.temporal.libs.datasets.datasets import make_dataset
from camera_baseline.temporal.libs.datasets.datasets import make_data_loader
from camera_baseline.temporal.libs.utils.train_utils import valid_one_epoch
from camera_baseline.temporal.libs.core.config import _update_config


def run_temporal(cfg, ckpt_folder, ckpt_freq, resume, rng_generator, run):
    ...
    for epoch in range(start_epoch, start_epoch + max_epochs):
        # train for one epoch
        t_loss = train_one_epoch(train_loader, model, optimizer, scheduler,epoch, model_ema=model_ema, clip_grad_l2norm=cfg['train_cfg']['clip_grad_l2norm'])
        ...
        v_loss, v_segments = valid_one_epoch(val_loader, model,epoch, evaluator=det_eval)
        ...
        if epoch == (start_epoch + max_epochs) - 1:
            ...
            val_mAP_post, _ , _= det_eval.evaluate(v_segments, cfg['score_threshold'])
